...
public class ViewDragHelper {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        int abs = Math.abs(i2);
        if (abs > 0) {
            i = 4 * Math.round(1148846080 * Math.abs(f / ((float) abs)));
        } else {
            i = (int) (((((float) Math.abs(i)) / ((float) i3)) + 1.0f) * 1132462080);
        }
    ...
    protected boolean canScroll(View view, boolean z, int i, int i2, int i3, int i4) {
        ...
        if (z) {
            if (!ViewCompat.canScrollHorizontally(view2, -i)) {
                if (ViewCompat.canScrollVertically(view2, -i2)) {
                }
            }
            return z2;
        }
        ...
        switch (actionMasked) {
            case 0:
                ...
                if ((this.mTrackingEdges & actionMasked) != 0) {
                    this.mCallback.onEdgeTouched(this.mTrackingEdges & actionMasked, motionEvent);
                    return;
                }
                return;
            case 1:
            ...
            case 5:
                ...
                if (this.mDragState == 0) {
                    ...
                    if ((this.mTrackingEdges & motionEvent) != 0) {
                        this.mCallback.onEdgeTouched(this.mTrackingEdges & motionEvent, actionMasked);
                        return;
                    }
                    return;
