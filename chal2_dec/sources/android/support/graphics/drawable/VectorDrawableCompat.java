...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    private static class VPathRenderer {
        ...
        private float getMatrixScale(Matrix matrix) {
            ...
            matrix = Math.max(hypot, hypot2);
            if (matrix > null) {
                return Math.abs(cross) / matrix;
            }
            return 0.0f;
        }
    }
    ...
    PorterDuffColorFilter updateTintFilter(PorterDuffColorFilter porterDuffColorFilter, ColorStateList colorStateList, Mode mode) {
        if (colorStateList != null) {
            if (mode != null) {
                return new PorterDuffColorFilter(colorStateList.getColorForState(getState(), null), mode);
            }
        }
        return null;
    }
    ...
    private void updateStateFromTypedArray(TypedArray typedArray, XmlPullParser xmlPullParser) throws XmlPullParserException {
        ...
        if (vPathRenderer.mViewportWidth <= 0.0f) {
            ...
            xmlPullParser.append(typedArray.getPositionDescription());
    ...
    private void inflateInternal(Resources resources, XmlPullParser xmlPullParser, AttributeSet attributeSet, Theme theme) throws XmlPullParserException, IOException {
        ...
        if (i != 0) {
            ...
            xmlPullParser = new StringBuilder();
            xmlPullParser.append("no ");
            xmlPullParser.append(stringBuffer);
            xmlPullParser.append(" defined");
            throw new XmlPullParserException(xmlPullParser.toString());
        }
    }
