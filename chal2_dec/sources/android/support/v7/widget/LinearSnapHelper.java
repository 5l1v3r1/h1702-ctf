...
public class LinearSnapHelper extends SnapHelper {
    ...
    public int findTargetSnapPosition(LayoutManager layoutManager, int i, int i2) {
        ...
        if (layoutManager.canScrollVertically()) {
            estimateNextPositionDiffForFling = estimateNextPositionDiffForFling(layoutManager, getVerticalHelper(layoutManager), 0, i2);
        ...
        if (layoutManager.canScrollVertically() == null) {
            estimateNextPositionDiffForFling = i;
        }
        if (estimateNextPositionDiffForFling == 0) {
            return -1;
        }
        position += estimateNextPositionDiffForFling;
        if (position < 0) {
            position = 0;
        }
    ...
    private int estimateNextPositionDiffForFling(LayoutManager layoutManager, OrientationHelper orientationHelper, int i, int i2) {
        ...
        if (computeDistancePerChild <= null) {
            return 0;
        }
        layoutManager = Math.abs(i[0]) > Math.abs(i[1]) ? i[0] : i[1];
        if (layoutManager > null) {
            return (int) Math.floor((double) (((float) layoutManager) / computeDistancePerChild));
        }
        return (int) Math.ceil((double) (((float) layoutManager) / computeDistancePerChild));
    }
    ...
    private float computeDistancePerChild(LayoutManager layoutManager, OrientationHelper orientationHelper) {
        ...
        if (view != null) {
            if (view2 != null) {
                layoutManager = Math.max(orientationHelper.getDecoratedEnd(view), orientationHelper.getDecoratedEnd(view2)) - Math.min(orientationHelper.getDecoratedStart(view), orientationHelper.getDecoratedStart(view2));
                if (layoutManager == null) {
                    return INVALID_DISTANCE;
                }
                return (INVALID_DISTANCE * ((float) layoutManager)) / ((float) ((i2 - i) + 1));
            }
        }
        return INVALID_DISTANCE;
    }
